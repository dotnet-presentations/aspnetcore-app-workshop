...
1. In the `BackEnd` folder, run `dotnet add package Microsoft.EntityFrameworkCore.SqLite` if you want to use SQLite during the workshop (e.g if not on Windows)
...
1. Add the following code to the top of the `ConfigureServices()` method in `Startup.cs`:
    ...
    ```
    > This code registers the `ApplicationDbContext` service so it can be injected into controllers. Additionally, it configures operating system specific database technologies and connection strings. To use SQLite add a reference to the NuGet package `Microsoft.EntityFrameworkCore.Sqlite`.
...
### Using Visual Studio
1. Right-click the `Controllers` folder and select Add/Controller. You'll be shown a prompt for setting up Scaffolding. Select "API Controller with actions, using Entity Framework".
