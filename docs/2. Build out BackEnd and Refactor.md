...
1. Move the following from Speaker.cs to ApplicationDbContext.cs
   ```
...
1. Add a reference to ConferenceDTO from BackEnd
...
1. Run the following commands in the command prompt for the BackEnd project:
   ```console
...
1. If present: Start by deleting the the attribute that reads `[Produces("application/json")]`. This attribute locks the controller's return type to JSON. By removing this attribute, we'll allow ASP.NET Core's content negotiatiation to output the response type the client requests.
