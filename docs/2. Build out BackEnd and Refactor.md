...
1. The data annotations references should be broken at this point, to resolve it, we need to add a nuget the missing NuGet package into the `ConferenceDTO` project.
1. Add a reference to the NuGet package `System.ComponentModel.Annotations` version `5.0.0`.
    > This can be done from the command line using `dotnet add package System.ComponentModel.Annotations --version 5.0.0`
...
1. Modify the `Speaker.cs` class we wrote previously to make the following two changes: update to the namespace to match our directory rename, and add a reference to the `SessionSpeaker` composite class:
